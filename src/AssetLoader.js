...
export async function loadAssets(levelPkg, tuningDoc) {
  ...
  const playerWalkImg = await loadImageAsync("assets/Walk.png");
  const playerJumpImg = await loadImageAsync("assets/Jump.png");
  const playerDeadImg = await loadImageAsync("assets/Dead.png");
  const playerAttackImg = await loadImageAsync("assets/Attack_3.png");

  const boarImg       = await loadImageAsync("assets/Idle_Werewolf.png");
  const boarWalkImg   = await loadImageAsync("assets/walk_Werewolf.png");
  const boarDeadImg   = await loadImageAsync("assets/Dead_Werewolf.png");
  const boarAttackImg = await loadImageAsync("assets/Attack_2.png");
  const leafImg = await loadImageAsync("assets/Slime3_Idle_body.png");
  const fireImg = await loadImageAsync("assets/fireSpriteSheet.png");

  const tile22Img = await loadImageAsync("assets/Tile_22.png");
  const groundTileImg = tile22Img;
  const groundTileDeepImg = tile22Img;
  const platformLCImg = tile22Img;
  const platformRCImg = tile22Img;
...
function defaultBoarAnis() {
  return {
    run:       { row: 0, frames: 11, frameDelay: 5 },
    throwPose: { row: 0, frames: 1,  frameDelay: Infinity },
    death:     { row: 0, frames: 2,  frameDelay: 10 },
  };
}
